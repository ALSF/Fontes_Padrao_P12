...
mBrowse(6,1,22,75,"CN9",,,,,,aCores,,,,,,,,cFilRev,,,,cFiltro)
Return
...
If Posicione("CN1",1,xFilial("CN1")+CN9->CN9_TPCTO,"CN1_ESPCTR") == '2'.And. SuperGetMV("MV_CNINTFS",.F.,.F.)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
CREATE PANEL oWizard;
HEADER STR0143;		//"Troca de Fornecedor/Cliente"
MESSAGE STR0159;	//"Informe os novos Fornecedores/Clientes que irão substituir os atuais."
...
EndIf
If ((cTpRev == DEF_ADITI .And. cEspec = '1') .OR. (cTpRev == DEF_READQ))
...
Else
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	If oGetDad1 == NIL
		oGetDad1 := MsNewGetDados():New(025,000, __DlgHeight(oWizard:oMPanel[9]), __DlgWidth(oWizard:oMPanel[9]),If((cModo $ "13" .AND. cTipRev == DEF_ADITI),GD_UPDATE+GD_DELETE+GD_INSERT,GD_UPDATE),"CN140VldIt()",,'+CNB_ITEM',,,999,,,"CN140DelGet()",oWizard:oMPanel[9],aHeaderIt,aColsIt)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If nPos = 0
	aAdd(aPlan,{cPlan,nVlTot,nVlTot,cCronog,CNAGWLoad(4,TRBCN9->CN9_NUMERO,cPlan)})
Else
	aDel(aPlan,nPos)
...
//Verifica se existe a periodicidade entre as parcelas
If CNV->(Dbseek(xFilial("CNV")+cContra+"   "+cCtbCron))
...
If !Empty(CN9->CN9_CODED)
	...
	BeginSQL Alias "TMPREAJ"
		SELECT SUM(CN9_VLREAJ) TOTREAJ
		FROM %Table:CN9% CN9
		JOIN %Table:CN0% CN0 ON
			CN0.%NotDel% AND 
			CN0.CN0_FILIAL = %xFilial:CN0% AND 
			CN0.CN0_CODIGO = CN9.CN9_TIPREV AND CN0_TIPO = '2'
		WHERE CN9.%NotDel% AND 
			CN9.CN9_FILIAL = %xFilial:CN9% AND
			CN9_NUMERO = %Exp:CN9->CN9_NUMERO% AND
			CN9_REVATU = %Exp:CN9->CN9_REVISA%
	EndSQL  
	GetLastQuery()
	
	nVlMax := (CN9->CN9_VLINI + TMPREAJ->TOTREAJ) * (1+nPerc) 
	nVlMin := (CN9->CN9_VLINI + TMPREAJ->TOTREAJ) * (1-nPerc)
...
If aPlan # NIL //-- Contrato fixo
	...
	For n1Cnt := 1 To Len(aPlan)
		If CNA->(dbSeek(xFilial("CNA")+cContra+cRevisa+aPlan[n1Cnt,1])) .And. cEspCtr == "1"
	 		...
	 		aTail(aItFor)[4] := Space(Len(CNC->CNC_FORNEC))
	 		aTail(aItFor)[5] := Space(Len(CNC->CNC_LJFORN))
	...
	While !CNC->(EOF()) .And. CNC->(CNC_FILIAL+CNC_NUMERO+CNC_REVISA) == xFilial("CNC")+cContra+cRevisa	
		...
		Else
			...
			While !CNA->(EOF()) .And. CNA->(CNA_FILIAL+CNA_CONTRA+CNA_REVISA) == xFilial("CNA")+cContra+cRevisa
				If CNA->(CNA_FORNEC+CNA_LJFORN) == CNC->(CNC_CODIGO+CNC_LOJA) .Or. CNA->(CNA_CLIENT+CNA_LOJACL) == CNC->(CNC_CLIENT+CNC_LOJACL)
...
PRIVATE nRevRtp := 3
PRIVATE lFisico := (Posicione("CN1",1,xFilial("CN1")+CN9->CN9_TPCTO,"CN1_CROFIS") == "1")
...
Local lRet 		:= .T.	
Local cContrato	:= "% CN9.CN9_NUMERO = '"+cContra+"' AND CN9.CN9_REVISA = '"+cRevisa+"' %" 


BeginSQL Alias cAliasSQL
SELECT 
		CN9.CN9_DTINIC as DATAI, CO0.CO0_LEI AS LEI
	FROM 
		%table:CN9% CN9
		JOIN %table:CO1% CO1 
		 ON 
				CO1.CO1_FILIAL = %xfilial:CO1% 
			AND CO1.CO1_CODEDT = CN9.CN9_CODED 
			AND CO1.CO1_NUMPRO = CN9.CN9_NUMPR
		JOIN %table:CO0% CO0 
		 ON 
		 		CO0.CO0_FILIAL = %xfilial:CO0% 
		 	AND CO0.CO0_REGRA = CO1.CO1_REGRA
	WHERE 
		%exp:cContrato%	
		AND CO1.CO1_REVISA = (SELECT MAX(CO1_2.CO1_REVISA) FROM %table:CO1% CO1_2
...
If !lRet 
	Help("GCP", 1, "MAXPRAZO",,"O prazo de vigência do contrato excedeu o limite de "+cPrazo+" estabelicido pela lei: "+cLei,1,1)
EndIf
