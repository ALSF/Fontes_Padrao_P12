...
@ 010, 051	SAY	OemToAnsi( STR0002 ); // "Arquivos de LayOut utilizados em SisPag"
				SIZE	150, 007;
				OF		oDlg;
				PIXEL 
...
@ 068, 058	BUTTON	OemToAnsi( STR0005 );  // "Restaura"
				SIZE		48, 11;
				OF			oDlg;
				PIXEL;
				ACTION	( 	nOpc1 := 2,;
...
@ 068, 109	BUTTON	OemToAnsi( STR0006 );  // "Excluir"
				SIZE		48, 11;
				OF			oDlg;
				PIXEL;
				ACTION	(	nOpc1 := 3,;
...
@ 068, 160	BUTTON	OemToAnsi( STR0007 );  // "Cancelar"
				SIZE		48, 11;
				OF			oDlg;
				PIXEL;
				ACTION	(	nOpc1 := 4,;
...
@ 090,057 Say OemToAnsi("Utilize os modelos disponíveis em") SIZE 100,7 OF oDlg PIXEL  // "Utilize os modelos disponíveis em"
                                                                                                  
@ 102,016 SAY oBtTdn PROMPT "<u>" + cTDN + "</u>" SIZE 200,011 OF oDlg HTML PIXEL
...
aDet :={{Space(20),Space(03),Space(03),Space(01),Space(60)}}
...
DBADDTREE	oTree;
				PROMPT 	PADR( OemToAnsi( STR0011 ), 20 ); 									// "Sispag       "
...
@ 003, 093 TO 130,324 OF oDlgMain PIXEL

@ 008, 115	BUTTON 	oBtn01;
...
ElseIf nPos == "144"
	...
	If Len( aGer ) > 0
		For ni = 1 to Len( aGer )
			If aGer[ ni, 1 ] == "5" .or. aGer[ ni, 1 ] == Chr(9)
				aAdd( aDet, {	aGer[ ni, 2 ], aGer[ ni, 3 ], aGer[ ni, 4 ], aGer[ ni, 5 ], aGer[ ni, 6 ] } )
			Endif
		Next
	Endif
